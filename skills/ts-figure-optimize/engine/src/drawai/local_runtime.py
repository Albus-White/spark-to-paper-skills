...
def _empty_torch_cache() -> None:
    ...
    # Guard with backend availability: on CPU/Linux torch builds torch.mps.empty_cache
    # exists but raises "Cannot execute emptyCache() without MPS backend" when called.
    if _torch_mps_available(torch):
