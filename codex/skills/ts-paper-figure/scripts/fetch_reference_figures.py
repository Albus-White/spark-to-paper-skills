...
def main() -> int:
    ...
    parser.add_argument("--max-papers", type=int, required=True,
                        help="model-selected fetch budget from the venue study and task relevance")
    ...
    try:
        ...
        print(json.dumps({"ok": False, "error": str(exc), "references_optional": True}))
        return 1
    if not papers:
        print(json.dumps({"ok": False, "error": "no papers supplied", "references_optional": True}))
        return 1
    ...
    for paper_index, paper in enumerate(papers[:args.max_papers]):
        ...
        for figure_index, (source_url, caption) in enumerate(ar5iv_figures(identifier, args.max_figures_per_paper)):
            ...
            candidates.append({
                "paper": paper.get("title"), "venue": paper.get("venue") or paper.get("journal"),
                "arxiv": identifier, "document_order": figure_index + 1, "caption": caption,
                "source_url": source_url, "file": str(target),
            })
    output = out_dir / f"{args.label}.candidates.json"
    output.write_text(json.dumps({
        "label": args.label, "candidates": candidates,
        "instruction": "The main model must inspect actual images and select only references relevant to this figure; an empty selection is valid.",
    }, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"ok": True, "n": len(candidates), "candidates_json": str(output), "references_optional": True}))
    return 0
