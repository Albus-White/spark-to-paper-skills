#!/usr/bin/env python3
"""Retrieve semantically close work for main-model novelty analysis.
...
kg_recall.py) and cosine-compares it against an available reference set:
  - PREFERRED: the abstracts in retrieved_papers.json (the actual prior works the story departs
    from) — embedded on the fly.
  - ELSE: the prebuilt PATTERN index kg/pattern_emb.npy (collision with reusable skeletons; the
    pattern-index hits are topical retrieval signals, not novelty verdicts).

Emits novelty_report.json with nearest works and cosine similarities. The main model compares actual
claims and evidence; this script never orders a pivot or declares an Idea novel.
...
def degrade(workdir, note, basis="unconfigured"):
    ...
    report = {
        "ok": True, "basis": basis,
        "max_similarity": None, "signal": "unavailable",
        "top_similar": [], "decision_authority": "main_model",
        "note": note,
    }
    (workdir / "novelty_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=1))
    print(json.dumps({"ok": True, "basis": basis, "signal": "unavailable", "note": note}))
    sys.exit(0)
...
def main():
    ...
    report = {
        "ok": True, "basis": basis,
        "max_similarity": round(max_sim, 5), "signal": "nearest_work_retrieval",
        "top_similar": [{"ref": lbl, "similarity": round(s, 5)} for lbl, s in scored[:5]],
        "decision_authority": "main_model",
        "instruction": "Compare claim, mechanism, assumptions, scope, and evidence against the retrieved works; cosine similarity is not a novelty score.",
    }
    (workdir / "novelty_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=1))
    print(json.dumps({"ok": True, "basis": basis, "max_similarity": report["max_similarity"],
                      "signal": "nearest_work_retrieval"}))
