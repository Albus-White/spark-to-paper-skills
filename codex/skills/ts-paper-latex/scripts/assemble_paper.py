...
substitution), and which style/asset files to copy. Missing templates fail closed.
...
Working dir layout (inputs):
  template.json       the explicitly selected template spec
...
def load_template(wd: Path):
    ...
    if not spec_path.is_file():
        raise ValueError("template.json is required; refusing to compile with a silent venue fallback")
    spec = json.loads(spec_path.read_text())
    name = spec.get("name")
    if not name:
        raise ValueError("template.json requires name")
    tmpl_name = (spec.get("engine") or {}).get("main_template", "main.tex.tmpl")
    tdir = wd if (wd / tmpl_name).exists() else (TEMPLATES_ROOT / name)
    if not (tdir / tmpl_name).is_file():
        raise ValueError(f"selected template source missing: {tdir / tmpl_name}")
    return spec, tdir


def assembly_input_hash(workdir: Path) -> str:
    files = []
    ...
    for directory in ("sections", "figures"):
        ...
        if root.is_dir():
            files.extend(path for path in root.rglob("*") if path.is_file() and ".pipeline" not in path.parts)
    payload = []
    for path in sorted(files):
        payload.append(path.relative_to(workdir).as_posix().encode() + b"\0" + hashlib.sha256(path.read_bytes()).digest())
...
def assemble(workdir: Path) -> dict:
    ...
    titles.update({sid: section.get("title", sid.replace("_", " ").title())
                   for sid, section in (bp.get("sections") or {}).items() if isinstance(section, dict)})
...
def compile_pdf(workdir: Path) -> dict:
    ...
    try:
        ...
        return {"compiled": False, "exit": 124, "error_count": 1,
                "error_tail": f"latexmk timed out after {exc.timeout}s"}
...
def main():
    ...
    result["input_hash"] = assembly_input_hash(workdir)
