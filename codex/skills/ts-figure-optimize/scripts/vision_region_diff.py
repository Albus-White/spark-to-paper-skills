...
(If you have no external vision endpoint, the skill executor — Codex — can read the region_crops.py
...
def main() -> int:
    ...
    if not key:
        print("FATAL: no vision API key (set OPENAI_API_KEY or ~/.codex/auth.json). "
              "Alternatively run scripts/region_crops.py and let the skill executor (Codex) judge the "
