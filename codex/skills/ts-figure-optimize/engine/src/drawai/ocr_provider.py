...
from .http_utils import model_busy_retry_after_seconds, remote_queue_max_polls, urlopen_direct_for_loopback
...
class OcrHttpStatusError(OcrProviderError):
    def __init__(self, message: str, *, http_status: int, body_excerpt: str = "",
...
class HttpJsonTransport:
        ...
        while True:
            ...
            try:
                ...
                if retry_after is not None:
                    queue_polls += 1
                    self._wait_for_model_queue(
                        path, timeout_s, queue_started, retry_after, queue_polls, exc
                    )
                    continue
    ...
    def _wait_for_model_queue(
        ...
        if (queue_polls >= remote_queue_max_polls()
                or time.monotonic() - queue_started + retry_after > queue_timeout_s):
            raise OcrHttpStatusError(
                _transport_error_message(
                    ...
                    cause=(f"queue_timeout_s={queue_timeout_s:g}, queue_polls={queue_polls}, "
                           f"queue_max_polls={remote_queue_max_polls()}"),
                ),
                http_status=cause.code,
                retryable=False,
            ) from cause
...
class RemotePaddleOcrProvider:
    def __init__(
        ...
        if not 1 <= self.retry_max_attempts <= 5:
            raise OcrProviderError("retry_max_attempts must be between 1 and 5")
...
def build_ocr_provider(
    ...
    if provider == "remote_paddleocr":
        ...
        retry_max_attempts = int(os.environ.get("DRAWAI_REMOTE_RETRY_MAX_ATTEMPTS", "3") or "3")
        if not 1 <= retry_max_attempts <= 5:
            raise OcrProviderError("DRAWAI_REMOTE_RETRY_MAX_ATTEMPTS must be between 1 and 5")
