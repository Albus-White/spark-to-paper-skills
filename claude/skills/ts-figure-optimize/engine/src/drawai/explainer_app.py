...
def _render_overview(
    ...
    if artifacts["ppt_after_preview"].get("exists"):
        gallery_items.append(_figure(artifacts["ppt_after_preview"], "优化后 PPT 预览", "PPT 元素优化阶段的渲染预览", resolver))
...
def _render_stage_ppt_optimization(
    ...
    body += _gallery(
        [
            ...
            _figure(artifacts["ppt_after_preview"], "优化后 PPT 预览", "PPT 元素优化阶段直接编辑后的渲染", resolver),
        ],
        class_name="gallery two",
    )
