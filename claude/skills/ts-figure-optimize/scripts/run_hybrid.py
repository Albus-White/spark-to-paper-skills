#!/usr/bin/env python3
"""run_hybrid.py — explicit fidelity-first FALLBACK: preprocessing (IR) + HYBRID export.
...
Usage:
Run only after the user explicitly approves raster graphics + editable text instead of full
DrawAI reconstruction.
...
def _engine() -> Path:
    ...
    self-contained, deploy-anywhere skill. The engine shipped inside this skill is authoritative."""
...
def sh(cmd, cwd=None):
    try:
        ...
        r = subprocess.CompletedProcess(cmd, 124, stdout, stderr + "\ncommand timed out after 1800s")
    if r.stdout:
        print(r.stdout[-1200:])
