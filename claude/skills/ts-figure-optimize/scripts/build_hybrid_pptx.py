...
Usage:
  python build_hybrid_pptx.py --source S.png --ocr ocr_boxes.json [--box-ir box_ir.json] \
      ...
      [--out-bg bg.png] [--report report.json] [--font /path/Times.ttf]
...
def write_pptx(bg_path, runs, W, H, out_pptx, font_path, font_family):
...
def write_svg_pdf(bg_path, runs, W, H, out_svg, out_pdf, font_family):
    esc = html.escape
    ...
    for r in runs:
        ...
        for seg, kind in r["segs"]:
            ...
            cur = tgt
        parts.append(f'<text x="{r["x"]}" y="{base_y:.1f}" font-family="{esc(font_family)}, sans-serif" '
...
def main():
    ...
    ap.add_argument("--font-family", default="DejaVu Sans",
                    help="venue-appropriate family; defaults to a portable neutral sans font")
    ...
    bg.save(bg_path)
    write_pptx(bg_path, runs, W, H, a.out_pptx, font_path, a.font_family)
    if a.out_pdf or a.out_svg:
        write_svg_pdf(bg_path, runs, W, H, a.out_svg or None, a.out_pdf or None, a.font_family)
