#!/usr/bin/env python3
"""plot_results.py — render a RESULTS figure by running a Codex-authored matplotlib script.

DATA-AWARE mode only. Results plots must be numerically exact, so they are drawn from CODE
(matplotlib), never from an image model. The design is deliberately schema-agnostic: Codex has
...
def main():
    ...
    ap.add_argument("--script", required=True, help="Codex-authored matplotlib script")
    ...
    ns = {"plt": plt, "np": np, "json": json, "Path": Path, "OUT": str(out),
          # house-style helpers, available to the Codex-authored plot script:
    ...
    try:
        exec(compile(script.read_text(), str(script), "exec"), ns)   # trusted: Codex-authored
