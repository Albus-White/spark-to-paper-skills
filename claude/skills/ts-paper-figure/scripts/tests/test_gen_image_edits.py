...
def _png_bytes(colour=(20, 40, 80)):
    buf = io.BytesIO()
    Image.new("RGB", (32, 24), colour).save(buf, format="PNG")
    return buf.getvalue()
...
def test_render_routes_to_edits_when_refs_present(monkeypatch, tmp_path):
    ...
    ref = tmp_path / "r.png"; ref.write_bytes(_png_bytes())
    out = tmp_path / "o.png"
...
def test_required_reference_missing_fails_closed(monkeypatch, tmp_path):
    monkeypatch.setenv("TS_FIG_API_KEY", "k"); monkeypatch.setenv("TS_FIG_BASE_URL", "https://x/v1")
    result = gi.render("P", tmp_path / "out.png", references=[str(tmp_path / "missing.png")],
                       require_reference=True)
    assert not result["ok"] and "missing" in result["error"]


def test_render_rejects_retry_budget_above_five(tmp_path):
    result = gi.render("diagram", tmp_path / "out.png", retries=6)
