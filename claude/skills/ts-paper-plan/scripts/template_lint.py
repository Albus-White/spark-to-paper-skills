...
def main():
    ...
    for s in sections:
        w = s.get("words")
        if w is not None and not (isinstance(w, (list, tuple)) and len(w) == 2 and w[0] <= w[1]):
            issues.append(f"section {s.get('id')}: optional words must be [min,max] with min<=max, got {w}")
        if s.get("roles") is not None and not isinstance(s.get("roles"), list):
            issues.append(f"section {s.get('id')}: roles must be a list")
