...
Combines: per-paper extractions (Codex wrote the story-first pattern fields), the cluster
assignments (cluster.py), and the per-cluster names/summaries (Codex wrote these), into the
...
Inputs:
  ...
  --cluster-meta meta.json {cluster_id:{name,summary,llm_enhanced_summary,tier}}  (Codex; optional)
